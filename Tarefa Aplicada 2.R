...
# Q3 - 10000 replica??es de um PA de 199 passos;
#      guardar: A) n de troca de sinais
#               B) o m?x atingido por cada passeio
...
# 4 - fun??o  q constr?i um P.A. em Z2, com n passos.
#     A fu??o de ve retornar um gr?fico e o estado final; 
...
Q4 = function(n){
     ...
     v[1] = 0
     z[1] = 0
     for(i in 2:n){                       # começa com 2. pois o primeiro momento já esta em (0,0)
         m = runif(1)
         if(m<0.5){                       # se m < 0.5, se move na horizontal
            m=runif(1)                     
            if(m<0.5){x=x+1}else{x=x-1}   # se m < 0.5, se move para direita, senão, para a esquerda 
            v[i] = x
            if(i == 1){z[i] = 0}else      # a coordenada vertical se mantém
                      {z[i] = z[i-1]}
            }else{                        # se m > 0.5, se move na vertical 
            m=runif(1)  
            if(m<0.5){y=y+1}else{y=y-1}   # se m < 0.5, se move para cima, senão, para baixo
            z[i] = y
            if(i == 1){v[i] = 0}else      # a coordenada horizontal se mantém
                      {v[i] = v[i-1]}
     }}
     plot(v,z, type = "l")                # plotando o gráfico em linhas
}
...
# 6 - graf. estrela at? repetir um viz
...
# 7 - graf. estrela at? atingir tds os viz:
...
# 8 - P.A. circular:
q8 = function(r,n,p){
     c = 1                               # começando no primeiro vizinho (vizinho 1)
     f = rep(0,r)                        # vetor para guardar os vizinhos visitados
     for(i in 1:n){
         m = runif(1)
         if(m<p){c = c + 1}else{c = c - 1}    # pulando de vizinho em vizinho
         if(c > r){c = 1}                     # se maior que r, volta pro primeiro vizinho
         if(c == 0){c = r}                    # se menor que 1, vai para o último vizinho
         f[c] = f[c] + 1                      # avisando que tal vizinho foi vizitado
         if(f[c] == 2){f[c] = 1}              # anulando as segundas visitas
     }
     cat(sum(f))    # retornando o número de vizinhos visitados; (onde f[i] = 1)
}
...
q9 = function(i,r,p){
     ...
     repeat{
           m = runif(1)
           if(m<p){i = i +1}else{i = i -1}   # se o jogador ganha ou perde
           v[j] = i                          # armazenando a informação
           j = j + 1                         # proseguindo com o passeio
           if(i == 0 || i == r)break         # se atingir 0 ou r, o jogador para;
           }
     cat("Sn:",i)
     plot(v)
}
...
q10 = function(i,r,p,n){
     ...
     j = 1
     q = 1
     for(g in 1:n){                  # similar ao código anterior, porém com n passeios aleatórios
     t = 0   
     repeat{                                  
           m = runif(1)
